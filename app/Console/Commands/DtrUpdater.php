...
class DtrUpdater extends Command
{
    protected $signature = 'dtr {id}';
    ...
    public function handle()
    {
        $id = $this->argument('id');
            ...
            if (!$dtr) {
                $this->error('DTR not found.');
                return Command::FAILURE;
            }

            $user = UserOrganization::with('shift.times')
            ->where('user_id', $dtr->user_id)
            ->first();

            if (!$user || !$user->shift) {
                $this->error('User shift not found.');
                return Command::FAILURE;
            }
            ...
            $hours = $user->shift->hours;
                ...
                }elseif ($isHalfDayPm && $dtr->am_in_at) {
                    $updates['is_halfday'] = 1;
                    $updates['hours'] = $hours/2;
                    $am_in_at = json_decode($dtr->am_in_at);
                ...
                $this->info('DTR updated successfully.');
